...
首次启用或正常重开后可能需要重启当前宿主应用。`start` 返回此提示时，按 `AGENTS.md` 的“重启与任务衔接”说明原因并引导用户：保存工作、等待当前任务结束，再从启动台或 Dock 打开「豆包换肤」App（推荐，无需确认）；也可以双击桌面「豆包换肤」文件夹里的 `启动豆包工作.command`（需要重启时询问确认）。安装版不要求用户提前手动退出应用；App 不存在时按指南提供桌面入口。当前版本不承诺跨重启续接任务；不要自行执行 `--force`、结束进程或反复重试来隐藏这个限制。
