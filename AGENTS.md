...
安装时会自动创建「豆包换肤」App（`~/Applications/豆包换肤.app`，可拖到 Dock）。**推荐使用 App 启动**，它会智能判断：应用未运行时直接启动带皮肤；启动成功后激活窗口；已运行但启动失败时自动重试一次，并在需要时直接重启，无需确认。

使用 App 时，按以下 2 步向用户说明：

1. 保存工作，等待当前 Agent 任务结束。
2. 从启动台、应用程序文件夹或 Dock 打开「豆包换肤」App；App 会自动处理重启，无需确认。

如果用户偏好终端脚本方式，**严格按以下 3 步向用户说明，不要增加、删减或改写步骤**：

1. 保存工作，等待当前 Agent 任务结束。
2. 去桌面打开「豆包换肤」文件夹，双击 `启动豆包工作.command`；终端问是否重启时输入 `y` 回车。
3. 豆包工作自动重新打开后，回到原对话继续。

**关键约束（必须遵守）：**

- **不要让用户手动完全退出豆包工作。** 启动入口会自动处理退出和重启，手动先退反而可能导致脚本找不到进程。
- **优先推荐「豆包换肤」App。** App 无需确认、可拖到 Dock，是日常启动的首选。只有 App 不存在或用户明确要求时，才引导桌面 `.command` 方式。
- **优先引导从桌面「豆包换肤」文件夹打开。** 只有桌面快捷方式确实不存在时，才给出 `~/Library/Application Support/DoubaoWorkSkin/启动入口/启动豆包工作.command` 作为备选路径。
- 双击入口使用上次成功选择的主题，首次默认「海风微语」。如果用户刚请求了其他主题但尚未生效，等用户返回对话后再执行 `start` 和 `verify`。
- 不要承诺重启后任务会自动继续，也不要在重启前报告皮肤已经生效。
- Agent 不应替用户打开 App 或运行带重启确认的快捷入口。
...
如果用户新开对话、Agent 缺少上下文，让其读取固定的安装版 `AGENTS.md`；用户也可双击桌面入口中的“复制换肤提示词.command”后粘贴。
