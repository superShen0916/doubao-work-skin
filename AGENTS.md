...
2. 去桌面打开「豆包工作皮肤」文件夹，双击 `启动豆包工作.command`；终端问是否重启时输入 `y` 回车。
3. 豆包工作自动重新打开后，回到原对话继续。

**关键约束（必须遵守）：**

- **不要让用户手动完全退出豆包工作。** 启动入口脚本会自动处理退出和重启，手动先退反而可能导致脚本找不到进程。
- **优先引导从桌面「豆包工作皮肤」文件夹打开。** 只有桌面快捷方式确实不存在时，才给出 `~/Library/Application Support/DoubaoWorkSkin/启动入口/启动豆包工作.command` 作为备选路径。
