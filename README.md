...
首次启用可能需要重启豆包工作。安装完成后，按提示保存工作，等待当前任务结束，再从 **启动台** 或 **应用程序** 文件夹打开「豆包换肤」App（安装时自动创建到 `~/Applications/豆包换肤.app`，可拖到 Dock）。App 会智能判断：应用未运行时直接启动带皮肤；已有皮肤时只激活窗口；需要重启时不询问直接重启。重新打开后回到原对话继续换肤。
