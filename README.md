...
下载新版完整包并重新运行安装脚本，或请 Agent 更新。个人皮肤和上次选择会保留；已有主题目录不会被覆盖，包括内置皮肤的个人副本。因此程序更新不会自动替换这些背景、配色和主题专属样式（`skin.css`）。需要同步内置主题的样式修复时，请让 Agent 先备份，再将相应修复合并到个人主题副本。更新完成后，通过桌面入口启动，或让 Agent 重新应用皮肤并验证。
